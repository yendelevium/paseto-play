...
PASETO tokens are made up of 3 (or 4) parts. The `v4` stands for the version of PASETO being used. `local` is the PASETOs purpose. It can be either `local` or `public`. Then the 3rd part is the `payload`. It's either encrypted (local) or signed (public). Finally, an optional 4th part is the `footer`. It can be used to store additional unencrypted metadata. The above token doesn't have the footer.
...
## SETUP AND TEST
Run `make build` and then `./paseto_play` and the server should be running on port 8080. The endpoints include `/local/encrypt`, `/local/decrypt`, `/public/sign` and `/public/verfiy` - which you can send a GET request to and see the magic happen. 